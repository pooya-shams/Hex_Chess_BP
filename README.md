...
شامل تنظیمات بازی به شکل تعدادی متغییر `static` است که در بخش‌های مختلف پروژه از آن‌ها برای تنظیم رفتار بازی استفاده می‌شود. تنظیمات فعلی شامل انواع رنگ‌های زمین بازی در محیط گرافیکی می‌شود و یک `boolean` که نشان دهندهٔ این است که آیا نوبت سیاه زمین بچرخد یا نه. این تنظیمات در فایل `config/config` ریخته و از آن خوانده می‌شوند و اگر موجود نباشد به تنظیمات پیشفرض برمی‌گردد.
